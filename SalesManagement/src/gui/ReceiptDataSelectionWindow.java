...
public class ReceiptDataSelectionWindow extends JDialog {
	...
	private SalesInputWindow inputDialog;
	private SalesReport selectedAgent;
	@SuppressWarnings("unused")
	private String fileType;
	private Receipt receipt;
	
	public ReceiptDataSelectionWindow(SalesInputWindow dialog, SalesReport agent, String fileTypeFlag) {
		inputDialog = dialog;
		selectedAgent = agent;
	...
	public void initialise(){
		...
		try{
			...
			JOptionPane.showMessageDialog(null,"agentNameTextField, NullPointerException");
	...
	protected void okButtonPressed(ActionEvent evt) {
		...
		if(shirtRadio.isSelected())
			shirtSales = selectedAgent.calculateSalesByKind("Shirts");		
		if(skirtRadio.isSelected()  )
			skirtSales = selectedAgent.calculateSalesByKind("Skirts");	
		if(coatRadio.isSelected())
			coatsSales = selectedAgent.calculateSalesByKind("Coats");		
		if(trousersRadio.isSelected())
			trousersSales = selectedAgent.calculateSalesByKind("Trousers");	
		if(commissionCheckBox.isSelected())
			commission = selectedAgent.calculateCommission();
		SalesOutputWindow rs = new SalesOutputWindow(this,selectedAgent, totalSales, totalItems, shirtSales, skirtSales, trousersSales, coatsSales, commission);
		rs.setVisible(true);
	...
	private boolean receiptDetailsTextFields() {
	...
	private void appendFile(){
		ReceiptInfoAppender Ap = selectedAgent.getFileAppender();
		Ap.setReceipt(receipt);
		Ap.appendFile();
	}
	...
	private void addReceipt(){
		...
		try{
			...
			JOptionPane.showMessageDialog(null,"mpika");

		}catch (NumberFormatException e){
			JOptionPane.showMessageDialog(null,"den mpika");
