...
public class SalesInputWindow extends JDialog {
	
	private static final long serialVersionUID = 1L;
	private final JPanel inputWindowPanel = new JPanel();
	private DefaultListModel <String> listModel = new DefaultListModel <String>();
	private JList <String> agentsList = new JList <String>();
	private Vector <SalesReport> allAgents;
	private  SalesReport agent = new SalesReport();
	private SalesReport selectedAgent = null;
	static SalesInputWindow dialog = new SalesInputWindow();
	...
	public SalesInputWindow(){		
		initialise();
	}
	
	public void initialise() {
		allAgents = new Vector <SalesReport>();
	...
	private void insertFromTXT(ActionEvent evt){
		...
		try{
			...
			agent = inputFileTXT.getSalesReport();
			agent.setFileTypeTXT();
			...
			if(agentDuplicate == true){
				JOptionPane.showMessageDialog(null,"agentDuplicate == true");
			}
			else{
	...
	private void insertFromHTML(ActionEvent evt3) {
		...
		try{
			...
			agent = inputFileHTML.getSalesReport();
			agent.setFileTypeHTML();			
	...
	private void insertFromXML(ActionEvent evt2){
		...
		try{
			...
			agent = inputFileXML.getSalesReport();
			agent.setFileTypeXML();
			...
			if(agentDuplicate == true){
				JOptionPane.showMessageDialog(null,"agentDuplicate");
			}
			else{
				listModel.addElement(agent.getName());
				agentsList.setModel(listModel);
	...
	private void okButtonPressed(ActionEvent evt) {
		if(agentsList.isSelectionEmpty()){
			JOptionPane.showMessageDialog(null,"agentsList.isSelectionEmpty");

		}
		else{
			ReceiptDataSelectionWindow sw = new ReceiptDataSelectionWindow(dialog,selectedAgent,fileTypeFlag);
